...
void World::render() {
	...
	for (std::vector<Object>::iterator o = objects.begin(); o != objects.end(); o++) {
		...
		glUniform3f(lightID, glm::sin(static_cast<float>(counter) / 1000.0) * 2,
							0.5,
							glm::cos(static_cast<float>(counter) / 1000.0) * 2);
...
void World::addTriangle(glm::vec3 a, glm::vec3 b, glm::vec3 c) {
	...
	// glGenBuffers(1, &o.elementBuffer);
...
void World::addCuboid(glm::vec3 a, glm::vec3 b, glm::vec3 c, glm::vec3 d) {
	...
	glm::vec3 g = e + (d - a);
	glm::vec3 h = g + (a - c);
	GLfloat * vertexData = new GLfloat[6*18];
	...
	GLfloat * normalData = new GLfloat[6*18];
	for (int i = 0; i < 3; i++) {
